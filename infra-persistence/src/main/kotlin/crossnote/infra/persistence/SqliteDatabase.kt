...
class SqliteDatabase(private val dbPath: Path) : AutoCloseable {
    ...
    init {
        ...
        val driverPrefix = if (isAndroid) "jdbc:sqldroid:" else "jdbc:sqlite:"
        
        if (isAndroid) {
            try {
                Class.forName("org.sqldroid.SQLDroidDriver")
            } catch (e: Exception) {
                // Driver might be loaded automatically or not available in current context
            }
        }

        connection = DriverManager.getConnection("$driverPrefix${dbPath.toAbsolutePath()}")
    ...
    private fun migrate() {
        connection.createStatement().use { st ->
            ...
            if (!columnExists("notebooks", "updated_at")) {
                st.execute("ALTER TABLE notebooks ADD COLUMN updated_at TEXT NOT NULL DEFAULT '';")
            }
